...
-- Joueurs
IF OBJECT_ID('dbo.Joueurs', 'U') IS NOT NULL
...
-- Jeux
IF OBJECT_ID('dbo.Jeux', 'U') IS NOT NULL
    DROP TABLE dbo.Jeux;
...
-- Plateformes
IF OBJECT_ID('dbo.Plateformes', 'U') IS NOT NULL
    DROP TABLE dbo.Plateformes;
...
ALTER TABLE dbo.Parties ADD CONSTRAINT FK_Parties_Joueurs
    FOREIGN KEY (id_joueur) REFERENCES dbo.Joueurs(id_joueur);
ALTER TABLE dbo.Parties ADD CONSTRAINT FK_Parties_Jeux
    FOREIGN KEY (id_jeu) REFERENCES dbo.Jeux(id_jeu);
ALTER TABLE dbo.Parties ADD CONSTRAINT FK_Parties_Plateformes
    FOREIGN KEY (id_plateforme) REFERENCES dbo.Plateformes(id_plateforme);

-- Achats
IF OBJECT_ID('dbo.Achats', 'U') IS NOT NULL
    DROP TABLE dbo.Achats;
...
ALTER TABLE dbo.Achats ADD CONSTRAINT FK_Achats_Joueurs
    FOREIGN KEY (id_joueur) REFERENCES dbo.Joueurs(id_joueur);
ALTER TABLE dbo.Achats ADD CONSTRAINT FK_Achats_Jeux
    FOREIGN KEY (id_jeu) REFERENCES dbo.Jeux(id_jeu);

-- Signalements
IF OBJECT_ID('dbo.Signalements', 'U') IS NOT NULL
    DROP TABLE dbo.Signalements;
...
ALTER TABLE dbo.Signalements ADD CONSTRAINT FK_Signalements_Joueurs
    FOREIGN KEY (id_joueur) REFERENCES dbo.Joueurs(id_joueur);

-- Sanctions
IF OBJECT_ID('dbo.Sanctions', 'U') IS NOT NULL
...
ALTER TABLE dbo.Sanctions ADD CONSTRAINT FK_Sanctions_Joueurs
    FOREIGN KEY (id_joueur) REFERENCES dbo.Joueurs(id_joueur);


============================================================
2. GÉNÉRATEUR DE DONNÉES PYTHON (fichier generate_data.py)
   Encodage latin-1 pour les CSV
============================================================
...
with open('data/joueurs.csv', 'w', newline='', encoding='latin-1') as f:
    ...
    for i in range(1, NB_JOUEURS+1):
        # Forcer 3 joueurs suspects pour la mission
        if i in [1, 2, 3]:
            pseudo = fake.user_name()
            pays = fake.country()
            date_insc = fake.date_between(start_date='-5y', end_date='today')
            heures = round(random.uniform(0, 50), 2)          # < 100h
            victoires = random.randint(200, 500)              # très haut
            defaites = random.randint(0, 50)                  # très bas
            precision = round(random.uniform(80, 99.99), 2)   # très précis
            tirs_tete = random.randint(500, 1000)
        else:
            pseudo = fake.user_name()
            pays = fake.country()
            date_insc = fake.date_between(start_date='-5y', end_date='today')
            heures = round(random.uniform(0, 5000), 2)
            victoires = random.randint(0, 500)
            defaites = random.randint(0, 500)
            precision = round(random.uniform(0, 100), 2)
            tirs_tete = random.randint(0, 1000)
...
with open('data/signalements.csv', 'w', newline='', encoding='latin-1') as f:
    ...
    # Forcer des signalements pour les joueurs suspects 1,2,3
    for i in range(1, NB_SIGNALEMENTS+1):
        if i <= 9:  # 3 signalements pour chacun des 3 suspects
...
with open('data/sanctions.csv', 'w', newline='', encoding='latin-1') as f:
    ...
    # Forcer une sanction pour les suspects 1,2,3
...
============================================================
3. COMMANDES BULK INSERT (à exécuter dans SSMS après création des tables)
   Avec CODEPAGE = '1252' pour lire correctement le latin-1
   ADAPTE LE CHEMIN VERS TON DOSSIER data/
============================================================

BULK INSERT dbo.Joueurs
FROM 'C:\chemin\vers\data\joueurs.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');

BULK INSERT dbo.Jeux
FROM 'C:\chemin\vers\data\jeux.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');

BULK INSERT dbo.Plateformes
FROM 'C:\chemin\vers\data\plateformes.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');

BULK INSERT dbo.Parties
FROM 'C:\chemin\vers\data\parties.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');

BULK INSERT dbo.Achats
FROM 'C:\chemin\vers\data\achats.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');

BULK INSERT dbo.Signalements
FROM 'C:\chemin\vers\data\signalements.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');

BULK INSERT dbo.Sanctions
FROM 'C:\chemin\vers\data\sanctions.csv'
WITH (FIELDTERMINATOR = ',', ROWTERMINATOR = '\n', FIRSTROW = 2, CODEPAGE = '1252');